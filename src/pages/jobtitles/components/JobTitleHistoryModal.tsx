...
const JobTitleHistoryModal: FC<Props> = ({ open, toggle, jobTitleId, jobTitleName }) => {
  ...
  const fetchHistory = async () => {
    const history = await getEntityHistory('JobTitle', jobTitleId)
  ...
  return (
    <Dialog fullWidth maxWidth='lg' open={open} onClose={toggle} dir={isRTL ? 'rtl' : 'ltr'}>
      <DialogTitle sx={{ textAlign: 'center', fontSize: '1.5rem !important' }}>
        ...
        {t('Job Title')} {t('History')}: {jobTitleName}
        <IconButton
          onClick={toggle}
          sx={{
            position: 'absolute',
...
export default JobTitleHistoryModal
